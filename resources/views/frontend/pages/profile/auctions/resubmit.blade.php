    ...
    <x-shadow-box shadowClass="shadow">
        ...
        <div class="row my-5">
            <div class="col-md-12">
                ...
                <form action="{{ route('profile.products.put.resubmit', $product->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <?php $submitButtonText = "Resubmit the product"; ?>
                    <x-product-submit-form :product="$product" :submitButtonLabel="$submitButtonText"/>
                </form>
            </div>
    </x-shadow-box>
</div>
